...
async function initializeMeta(lead: any, score: number) { const path = 'crm/lead-meta.json'; try { const info: any = await withBlobAuth((auth) => head(path, auth)); if (!info?.url) return; const current = await read(info.url); if (!current || typeof current !== 'object' || current[lead.id]) return; const now = new Date().toISOString(); current[lead.id] = { status: 'New', priority: priority(score), nextAction: 'Call', followUp: initialFollowUp(lead, score), note: 'Automatically initialized from new lead intake.', propertyType: lead.property_type || '', propertyName: lead.property_name || '', location: lead.location || '', budget: lead.budget || '', timeline: lead.timeline || '', callHistory: [], history: [{ id: `auto-${lead.id}`, at: now, action: 'Lead Automation', note: `New lead scored ${score}/100 · ${priority(score)} · Call · follow-up ${initialFollowUp(lead, score)}.` }] }; await withBlobAuth((auth) => put(path, JSON.stringify(current), { access: 'private', addRandomSuffix: false, allowOverwrite: true, contentType: 'application/json', ...auth })); } catch (error) { console.error('lead automation initialization error', error); } }
