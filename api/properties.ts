...
const RECOVERY_PROPERTIES: Property[] = [
  { id:'recovered-aminabad-710', title:'710 sq ft Flat', propertyType:'Flat', location:'Sector 1, Aminabad, Greater Noida', price:'₹40 Lakhs', minBudget:4000000, maxBudget:4000000, area:'710 sq ft', bedrooms:'', status:'Available', description:'Flat in Sector 1, Aminabad, Greater Noida. Seller: Arun.', createdAt:new Date().toISOString() },
  { id:'recovered-haridwar-110-bigha', title:'110 Bigha Commercial Land', propertyType:'Commercial Land', location:'Haridwar', price:'₹47 Lakhs per Bigha', minBudget:517000000, maxBudget:517000000, area:'110 bigha', bedrooms:'', status:'Available', description:'110 bigha commercial land in Haridwar. Demand: ₹47 Lakhs per bigha. Approx. total value: ₹51.70 Cr. Seller: Arun.', createdAt:new Date().toISOString() },
  { id:'recovered-faridabad-hospital', title:'100 Beds Hospital', propertyType:'Hospital', location:'Faridabad', price:'₹55 Cr · 3000 sq yard', minBudget:550000000, maxBudget:550000000, area:'3000 sq yard', bedrooms:'', status:'Available', description:'Operational 100-bed hospital. Seller: Rohit Sharma. Seller contact is kept private and should be shared directly when needed.', createdAt:new Date().toISOString() },
];
...
async function readLegacyProperties():Promise<Property[]>{const result=await get(PATH,{access:'private',...blobAuth});if(!result||result.statusCode!==200||!result.stream)throw new Error(`Blob read failed with status ${result?.statusCode??404}`);const text=await new Response(result.stream).text();const data=JSON.parse(text);if(!Array.isArray(data))throw new Error('Property inventory data is invalid');return data;}
async function readItemProperties():Promise<Property[]>{const blobs:any[]=[];let cursor:string|undefined;do{const page=await list({prefix:ITEM_PREFIX,cursor,...blobAuth});blobs.push(...(page.blobs||[]));cursor=page.hasMore?page.cursor:undefined;}while(cursor);const properties:Property[]=[];for(const blob of blobs){const result=await get(blob.pathname,{access:'private',...blobAuth});if(!result||result.statusCode!==200||!result.stream)continue;try{const value=JSON.parse(await new Response(result.stream).text());if(value&&typeof value==='object'&&value.id)properties.push(value as Property);}catch{}}return properties;}
async function readProperties():Promise<Property[]>{const items=await readItemProperties();if(items.length>0)return items;try{return await readLegacyProperties();}catch{return [];}}
async function itemPath(id:string){return `${ITEM_PREFIX}${encodeURIComponent(id)}.json`;}
async function ensureItemStorage(properties:Property[]){for(const property of properties)await put(await itemPath(property.id),JSON.stringify(property),{access:'private',addRandomSuffix:false,allowOverwrite:true,contentType:'application/json',...blobAuth});}
async function seedRecoveredPropertiesOnce(){const existing=await readProperties();
  // Self-heal: an old recovery marker must never suppress recovery when the live
  // item storage is empty. This prevents the CRM from showing “0 properties”
  // after a Blob/item migration or accidental item deletion.
  if(existing.length>0)return;
  const marker=await get(RECOVERY_MARKER,{access:'private',...blobAuth});
  const recovered=RECOVERY_PROPERTIES.map(p=>({...p,createdAt:p.createdAt||new Date().toISOString()}));
  if(marker?.statusCode===200){await ensureItemStorage(recovered);return;}
  await ensureItemStorage(recovered);
  await put(RECOVERY_MARKER,JSON.stringify({seededAt:new Date().toISOString(),ids:recovered.map(p=>p.id),version:2}),{access:'private',addRandomSuffix:false,allowOverwrite:true,contentType:'application/json',...blobAuth});
}
...
export default async function handler(request:any,response:any){if(!authorized(request))return send(response,401,{error:'Unauthorized'});try{if(request.method==='GET'){await seedRecoveredPropertiesOnce();return send(response,200,{properties:await readProperties()});}if(request.method==='POST'){const body=parseBody(request);const action=String(body.action||'upsert');const all=await readProperties();const itemBlobs=await list({prefix:ITEM_PREFIX,...blobAuth});if(!itemBlobs.blobs?.length&&all.length)await ensureItemStorage(all);
if(action==='uploadPhoto'){const id=String(body.id||'');const dataUrl=String(body.dataUrl||'');const property=all.find(p=>p.id===id);if(!property)return send(response,404,{error:'Property not found.'});if(!dataUrl.startsWith('data:image/'))return send(response,400,{error:'Please upload an image file.'});if(dataUrl.length>4_000_000)return send(response,413,{error:'Image is too large. Please choose a smaller photo.');}const match=dataUrl.match(/^data:(image\/(?:jpeg|jpg|png|webp));base64,(.+)$/);if(!match)return send(response,400,{error:'Unsupported image format.'});const ext=match[1].includes('png')?'png':match[1].includes('webp')?'webp':'jpg';const photoId=`${id}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;const pathname=`${PHOTO_PREFIX}${encodeURIComponent(photoId)}.${ext}`;const blob=await put(pathname,Buffer.from(match[2],'base64'),{access:'public',addRandomSuffix:false,contentType:match[1],...blobAuth});const photos=[...(property.photos||[]),blob.url].slice(-12);const updated={...property,photos};await put(await itemPath(id),JSON.stringify(updated),{access:'private',addRandomSuffix:false,allowOverwrite:true,contentType:'application/json',...blobAuth});return send(response,200,{ok:true,property:updated,properties:all.map(p=>p.id===id?updated:p)});}
