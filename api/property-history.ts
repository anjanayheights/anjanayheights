...
export default async function handler(request: any, response: any) {
  ...
  try {
    if (request.method === 'GET') {
      const result = await list({ prefix: 'crm/history/properties-', ...blobAuth });
      const snapshots = [];
      for (const blob of result.blobs) {
        try {
          const data = await readJson(blob.url);
          if (Array.isArray(data)) snapshots.push({ pathname: blob.pathname, uploadedAt: blob.uploadedAt, properties: data });
    ...
    if (request.method === 'POST') {
      ...
      const current = await readJson((await list({ prefix: CURRENT, ...blobAuth })).blobs[0]?.url || CURRENT);
