...
export default function RevenueDashboard() {
  ...
  const monthlyTrend = useMemo(() => Array.from({ length: 6 }, (_, index) => {
    ...
    const value = closed.filter(x => { const cd = new Date(x.meta.closedDate || ''); return !Number.isNaN(cd.getTime()) && cd.getMonth() === d.getMonth() && cd.getFullYear() === d.getFullYear(); }).reduce((s, x) => s + amount(x.meta.dealValue), 0);
