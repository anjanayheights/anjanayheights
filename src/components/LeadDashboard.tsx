...
export default function LeadDashboard() {
  ...
  const followUpToday = leads.filter((lead) => getMeta(lead.id).followUp === today && !['Closed', 'Lost'].includes(getMeta(lead.id).status)).length;
  const overdueFollowUps = leads.filter((lead) => { const d = getMeta(lead.id).followUp; return d && d < today && !['Closed', 'Lost'].includes(getMeta(lead.id).status); }).length;
  if (!loggedIn) return (
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] p-4 md:p-8"><div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:justify-between gap-4 mb-8"><div><h1 className="text-3xl font-bold text-[#1A365D]">Anjanay Heights</h1><p className="text-gray-500 mt-1">Lead Management Dashboard</p></div><button onClick={() => void loadLeads()} disabled={loading} className="bg-[#1A365D] text-white px-5 py-3 rounded-xl font-semibold disabled:opacity-60">{loading ? 'Refreshing...' : 'Refresh Leads'}</button></div>
      {error && <div className="bg-red-50 text-red-700 rounded-xl px-4 py-3 mb-5 text-sm">{error}</div>}
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-9 gap-3 mb-6"><div className="bg-white rounded-2xl p-4 shadow"><p className="text-gray-500 text-sm">Total</p><p className="text-2xl font-bold text-[#1A365D]">{leads.length}</p></div>{STATUSES.map((status) => <button key={status} onClick={() => setStatusFilter(status)} className={`text-left bg-white rounded-2xl p-4 shadow ${statusFilter === status ? 'ring-2 ring-[#1A365D]' : ''}`}><p className="text-gray-500 text-sm">{status}</p><p className="text-2xl font-bold text-[#1A365D]">{statusCounts[status] || 0}</p></button>)}<button onClick={() => setSearch(today)} className="text-left bg-white rounded-2xl p-4 shadow"><p className="text-gray-500 text-sm">Follow-up today</p><p className="text-2xl font-bold text-[#1A365D]">{followUpToday}</p></button></div>
      {overdueFollowUps > 0 && <button onClick={() => setSearch('')} className="w-full text-left bg-amber-50 border border-amber-200 text-amber-800 rounded-xl px-4 py-3 mb-5">⚠️ <strong>{overdueFollowUps}</strong> overdue follow-up{overdueFollowUps > 1 ? 's' : ''}. Review the leads below and update the next action.</button>}
