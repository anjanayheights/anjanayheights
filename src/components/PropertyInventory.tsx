...
export default function PropertyInventory() {
  ...
  async function addFaridabadHospital() {
    const existing = properties.find(p => p.title.toLowerCase() === '100 beds hospital' && p.location.toLowerCase().includes('faridabad'));
    if (existing) { setError('100 Beds Hospital in Faridabad is already in inventory.'); return; }
    setLoading(true); setError('');
    try {
      const hospital = {
        title: '100 Beds Hospital', propertyType: 'Hospital', location: 'Faridabad',
        price: '₹55 Cr', minBudget: 550000000, maxBudget: 550000000,
      ...
      const r = await fetch('/api/properties', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${password}` }, body: JSON.stringify({ action: 'upsert', ...hospital }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Unable to add hospital');
      setProperties(d.properties || []);
    } catch (e: any) { setError(e.message || 'Unable to add hospital'); }
    finally { setLoading(false); }
  }
