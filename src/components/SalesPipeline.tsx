...
export default function SalesPipeline() {
  ...
  function getMeta(id: string) { return { ...DEFAULT_META, ...(meta[id] || {}) }; }
  async function load() {
  ...
  async function moveLead(lead: Lead, status: string) {
    ...
    const nextAction = status === 'Closed' || status === 'Lost' ? 'No Action' : status === 'Site Visit' ? 'Site Visit' : status === 'Negotiation' ? 'Follow-up' : status === 'New' ? 'Call' : 'Follow-up';
    const next = { ...current, status, nextAction };
    setMoving(lead.id); setError('');
    setMeta(prev => ({ ...prev, [lead.id]: next }));
    try {
      const response = await fetch('/api/lead-meta', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${password}` }, body: JSON.stringify({ leadId: lead.id, meta: next }) });
      if (!response.ok) { const r = await response.json().catch(() => ({})); throw new Error(r.error || 'Could not update stage'); }
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not update stage');
      setMeta(prev => ({ ...prev, [lead.id]: current }));
    } finally { setMoving(''); }
  }
