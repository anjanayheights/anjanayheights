...
const DEFAULT_META: LeadMeta = { status: 'New', followUp: '', note: '', priority: 'Warm', nextAction: 'Call', propertyType: '', location: '', budget: '', timeline: '', visitDate: '', visitTime: '', visitedProperty: '', visitFeedback: '', visitInterest: '', dealValue: '', closingProbability: '' };
...
export default function SalesPipeline() {
  ...
  async function load() {
    const activePassword = password || sessionStorage.getItem('anjanay-heights-crm-password') || '';
    if (!activePassword) { setLoggedIn(false); setError('Please enter your dashboard password.'); return; }
    if (!password) setPassword(activePassword);
  ...
  useEffect(() => {
    ...
    if (saved) { setPassword(saved); void load(); }
  }, []);
