...
export default function AiLeadAssistant() {
  ...
  async function runAssistant() {
    ...
    try {
      ...
      const response = await fetch('/api/ai-lead-assistant', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${password}`, Accept: 'application/json' },
  ...
  if (!loggedIn) return (
    <div className="min-h-screen bg-[#F5F7FA] flex items-center justify-center px-4">
      <form onSubmit={e => { e.preventDefault(); void checkPassword(); }} className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8">
        ...
        <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter password" className="w-full border rounded-xl px-4 py-3" autoComplete="current-password" />
        {error && <p className="text-red-600 text-sm mt-3">{error}</p>}
        <button type="submit" disabled={loading} className="w-full mt-5 bg-[#1A365D] text-white rounded-xl py-3 font-semibold">{loading ? 'Verifying...' : 'Open AI Assistant'}</button>
      </form>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#F5F7FA] p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <button onClick={() => { setLoggedIn(false); setPassword(''); setText(''); setError(''); }} className="bg-white border px-4 py-2 rounded-xl font-semibold">Logout</button>
        </div>
