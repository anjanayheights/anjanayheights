...
const addDays=(days:number)=>{const d=new Date();d.setDate(d.getDate()+days);return new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit'}).format(d)};
...
export default function FollowupAutomation(){
 ...
 async function completeFollowup(id:string,days:number){
  ...
  const history=[...(current.history||[]),{action:`Follow-up completed · next follow-up ${next}`,at:stamp()}].slice(-30);
  ...
  try{
   ...
   if(!r.ok) throw new Error('Could not save follow-up automation');
  }catch(e){setError(e instanceof Error?e.message:'Could not save follow-up automation');}
  finally{setSaving('');}
 }
