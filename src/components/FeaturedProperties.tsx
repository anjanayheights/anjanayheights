...
function whatsappUrl(p: typeof properties[number]) {
  const text = `Hello Anjanay Heights, I am interested in ${p.name} in ${p.location} (${p.price}, ${p.area}). Please share the property details and site visit availability. My budget is:`;
  return `https://wa.me/919289771222?text=${encodeURIComponent(text)}`;
}
    ...
    <section className="py-20 bg-[#F9F9F7] border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="border-l-4 border-[#C2A36B] pl-8 py-2"><div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-4">Live Inventory Showcase</div><h2 className="text-3xl md:text-4xl font-serif text-[#1A365D] font-light">Exclusive Properties</h2></div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6"><AnimatePresence>{filteredProperties.map((prop, idx) => (
          <motion.div layout key={prop.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 0.4, delay: idx * 0.1 }} className="group relative overflow-hidden border border-gray-200 bg-white">
            <div className="aspect-[3/4] relative overflow-hidden"><img src={prop.image} alt={prop.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" /><div className="absolute inset-0 bg-[#1A365D]/90 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-between p-6"><div><div className="text-[10px] font-bold uppercase tracking-widest text-[#C2A36B] mb-2">{prop.type}</div><h3 className="text-xl font-serif text-white mb-1">{prop.name}</h3><p className="text-sm text-white/80">{prop.location}</p></div><div><div className="space-y-3 border-t border-white/20 pt-4 mb-6"><div className="flex justify-between text-xs text-white/90"><span>Area</span><span className="font-medium">{prop.area}</span></div><div className="flex justify-between text-xs text-white/90"><span>Price</span><span className="font-bold text-[#C2A36B]">{prop.price}</span></div></div><a href={whatsappUrl(prop)} target="_blank" rel="noreferrer" className="block w-full text-center border border-[#C2A36B] text-[#C2A36B] py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-[#C2A36B] hover:text-[#1A365D]">WhatsApp for Details</a></div></div></div>
