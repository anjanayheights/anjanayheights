...
export default function LeadDealConversion() {
  ...
  async function transition(lead: Lead, status: string, nextAction: string) {
    ...
    const history = [...(old.history || []), { action: `Status: ${status}`, at: new Date().toISOString() }].slice(-20);
  ...
  return <section className="max-w-7xl mx-auto px-4 md:px-8 pt-6"><div className="rounded-2xl bg-white border shadow-sm overflow-hidden"><div className="p-5 border-b flex items-center justify-between"><div><h2 className="text-xl font-bold text-[#1A365D]">🔗 Lead → Deal Conversion</h2><p className="text-sm text-slate-500 mt-1">One-click stage change + follow-up + activity history.</p></div><button onClick={() => void load()} className="rounded-lg border px-3 py-2 text-sm font-semibold">{loading ? 'Loading…' : '↻ Refresh'}</button></div><div className="p-4 space-y-3">{active.map(lead => { const m = meta[lead.id] || {}; const value = Number(String(m.dealValue || '').replace(/[^0-9.]/g, '')) || 0; const commission = value * (Number(m.sellerCommissionRate || 1) / 100); return <div key={lead.id} className="border rounded-xl p-3"><div className="flex flex-wrap justify-between gap-2"><div><b>{lead.name || 'Lead'}</b><div className="text-xs text-slate-500">{lead.property_type || 'Property'} · {lead.location || 'Location'} · {lead.budget || 'Budget'}</div></div><span className="text-xs font-semibold rounded-full border px-2 py-1">{m.priority || 'Warm'} · {m.status || 'New'}</span></div><div className="flex flex-wrap gap-2 mt-3"><button onClick={() => void transition(lead, 'Contacted', 'Follow-up')} className="rounded-lg border px-3 py-2 text-xs font-semibold">✅ Contacted</button><button onClick={() => void transition(lead, 'Interested', 'Follow-up')} className="rounded-lg border px-3 py-2 text-xs font-semibold">👍 Interested</button><button onClick={() => void transition(lead, 'Site Visit', 'Site Visit')} className="rounded-lg border px-3 py-2 text-xs font-semibold">📅 Site Visit</button><button onClick={() => void transition(lead, 'Negotiation', 'Negotiation')} className="rounded-lg border px-3 py-2 text-xs font-semibold">🤝 Negotiation</button><button onClick={() => void transition(lead, 'Closed', 'Follow-up')} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white">✅ Close Deal</button></div>{m.status === 'Closed' && <div className="mt-2 text-xs text-emerald-700">Commission workflow started: {value ? money(commission) : 'Deal value pending'} · Pending</div>}</div>})}{active.length === 0 && <div className="text-center text-sm text-slate-500 py-6">No active leads available.</div>}</div></div></section>;
}
