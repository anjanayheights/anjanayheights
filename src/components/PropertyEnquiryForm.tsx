      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="grid lg:grid-cols-[0.85fr_1.15fr] gap-8 items-start">
          ...
          <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-5 md:p-7 shadow-sm space-y-4">
            ...
            <div><label htmlFor="property-enquiry-property" className="block text-sm font-medium text-slate-700">Property</label><select id="property-enquiry-property" name="message" defaultValue="" required className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="" disabled>Select a property</option>{PROPERTY_OPTIONS.map((property) => <option key={property} value={property}>Interested in {property}</option>)}</select></div>
            <div className="grid md:grid-cols-2 gap-4">
